...
export async function POST(request: Request) {
  ...
  const kind = body?.kind === "call" ? "call" : "message";
  ...
  for (const s of subs || []) {
    try {
      await webpush.sendNotification({ endpoint: s.endpoint, keys: { p256dh: s.p256dh, auth: s.auth } } as any, JSON.stringify({ title, body: text, url: "/", tag: `ubridge:${kind}:${to}`, kind }));
      sent++;
    } catch {}
  }
